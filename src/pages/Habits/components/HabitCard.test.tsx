...
test("if streak == goal, reset button is gone, replaced with check icon",()=>{
    ...
    render(<BrowserRouter><HabitCard openModal={jest.fn()} key={"1"} deleteHabit={jest.fn()} doHabit={jest.fn()} resetStreak={jest.fn()} index={1} currentDate={new Date("2023-09-02")} habitProp={habitProp}/></BrowserRouter>)
...
test("if streak == goal, check button is gone, replaced with check icon",()=>{
    ...
    render(<BrowserRouter><HabitCard openModal={jest.fn()} deleteHabit={jest.fn()} key="1" doHabit={jest.fn()} resetStreak={jest.fn()} index={1} currentDate={new Date("2023-09-02")} habitProp={habitProp}/></BrowserRouter>)
...
test("if today already done, checkbox is checked and cannot be unchecked",()=>{
    const habitProp:HabitProp = {
        ...
        isDone:false}
    const currentDate = new Date("2023-09-02")
    render(<BrowserRouter><HabitCard openModal={jest.fn()} deleteHabit={jest.fn()} habitProp={habitProp} doHabit={jest.fn()} index={1} resetStreak={jest.fn()} currentDate={currentDate} key={"1"}/></BrowserRouter>
    )
